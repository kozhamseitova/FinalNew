...
<c:forEach items="${events}" var="event">
        ...
        <div class="-my-8 news_bord" style="border-bottom: 3px solid #EE445F; margin-top: 2%; padding: 5%">
            <div class="py-8 flex flex-wrap md:flex-no-wrap">
                <div class="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                    <span class="tracking-widest font-medium title-font text-gray-900 span_a" style="border-right: 2px solid #EE445F; padding: 8px;">Event id: ${event.id}</span>
                    <span class="mt-1 text-gray-500 text-sm span_a" style="margin-left: 8px;">${event.time}</span>
                </div>
                <div class="md:flex-grow theme_news2">
                    <h2 class="text-2xl font-medium text-gray-900 title-font mb-2" style="color: white">${event.title}</h2>
                    <p class="leading-relaxed">${event.description}</p>
                    <%
                        if(session.getAttribute("status")!=null){
                    %>
                    <a class="text-green-500 inline-flex items-center mt-4" id="r" href="#">REMOVE
